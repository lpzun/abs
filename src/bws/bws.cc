...
bool BWS::reachability_analysis_via_bws(const string& filename,
	...
	if (s_initl.find('|') != std::string::npos) {
		this->initl_TS = utils::create_thread_state_from_str(s_initl);
	} else {
		ifstream in(s_initl.c_str());
		if (in.good()) {
			string s_ts;
			std::getline(in, s_ts);
			this->initl_TS = utils::create_thread_state_from_str(s_ts);
			in.close();
		} else {
	...
	/// setup the final thread state
	if (s_final.find('|') != std::string::npos) {
		this->final_TS = utils::create_thread_state_from_str(s_final);
	} else {
		ifstream in(s_final.c_str());
		if (in.good()) {
			string s_ts;
			std::getline(in, s_ts);
			this->final_TS = utils::create_thread_state_from_str(s_ts);
			in.close();
		} else {
			throw bws_runtime_error("target state file does not find!");
		}
	}

	cout << initl_TS << " " << final_TS << "\n";
